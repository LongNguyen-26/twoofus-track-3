...
- **Pod session 21/07 (4090 EU-RO-1, pod z7rhnlq02iayb6, ~1h): stock spec-decode tracks were dead on v0.25.1.** Battery `tools/runpod/draft_battery_r2.sh` D0–D10, logs on network volume ka4m1yetr4 (`/workspace/repo/results_draft_*`):
  ...
  - Verdict as of 21/07: ngram, ngram_gpu, and draft-model routes were closed; suffix was unresolved. **This verdict was reopened on 24/07 by the no-build-isolation custom-image workflow below.**
...
- **Suffix track reopened 24/07 — implementation complete, pod validation pending.** `arctic-inference==0.1.1` can be built against vLLM 0.25.1's existing PyTorch with `--no-build-isolation`; keep `ARCTIC_INFERENCE_ENABLED=0` so Arctic's old patch plugin stays disabled while vLLM uses its built-in `SuffixDecodingProposer`. The workflow includes D0A/S4/S8/S16/D0B, exact greedy equivalence, 28k needle, Prometheus acceptance counters, and result summarization. Entry point: `tools/docs/SUFFIX_RUNPOD.md`.
- **Final-5 REOPENED on 24/07** because the published cutoff is above every existing result. Retain 020, 024, and 018 BF16 insurance provisionally; reserve at least two slots for validated suffix/draft/quant candidates.
- **Current plan (24–30/07)**: ① run the suffix smoke and full battery on a pod; ② only publish/submit a suffix candidate if equivalence is 6/6, needle is not worse than D0, and TPOT improves repeatably by ≥15%; ③ bracket candidates with exact D0 baselines to measure drift; ④ ask BTC for failed-count breakdown; ⑤ submit early when a candidate clears the gate.
...
- `docker/lfm25-suffix/` — reproducible suffix custom-image Dockerfile plus a portal compose template. Do not create the next numbered submission until the pod battery selects S4/S8/S16.
