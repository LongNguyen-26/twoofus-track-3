...
- The public metadata marks **15 conversations / 90 rows** as `in_warmup: true`, followed by 55 conversations / 330 rows. **This marker is stale for the current harness: BTC scores all 70 conversations / 420 requests with no warm-up.** Arrivals span t=0..303s (Poisson after the first group, mean gap ≈4.75s, min 33ms).
...
- Concurrency remains low relative to round 1: on the current all-420 workload, simulated 1s service gives peak 6 / average 1.3 in flight; 2s gives peak 11 / average 2.5. **Per-request latency, not bulk throughput, is what scores.** Makespan is ≈324–330s at 1–2s service — far inside the 2700s job cap.
- Total current scored prefill ≈1.68M tokens naive (1.32M for the 330 rows without the stale marker). KV pool on 18GB with 2.4GB weights ≈1M+ tokens (12KB/token BF16, 6 full-attention layers × 8 KV heads × 64 head dim); capacity is still not the main constraint, and portal submit_021 proved fp8 KV is a latency regression.
...
- **Score budget after 020 (new harness)**: s_ttft(42) ≈ 0.84 — near its ceiling; s_tpot ≈ 0.42 (TPOT ~4ms) — still THE hole. TPOT 4→2ms ⇒ ~+18.5 pts (→ ~81). Stock flag tuning and every speculative-decoding route are now exhausted; the remaining high-upside track is reducing weight bandwidth per target step, starting with official vLLM in-flight BitsAndBytes W4.
- **Pod session 21/07 (4090 EU-RO-1, pod z7rhnlq02iayb6, ~1h): stock spec-decode tracks were dead on v0.25.1.** Battery `tools/runpod/draft_battery_r2.sh` D0–D10, logs on network volume ka4m1yetr4 (`/workspace/repo/results_draft_*`):
  ...
  - D9 (suffix, historical attempt): ordinary `pip install arctic-inference==0.1.1` failed while resolving isolated build dependencies. Source inspection on 24/07 found the build metadata pins `torch==2.7.0`; this was a packaging/build-isolation problem, not proof that the CPU suffix-tree extension itself is incompatible.
  - Verdict as of 21/07: ngram, ngram_gpu, and draft-model routes were closed; suffix was unresolved. The 24/07 no-build-isolation workflow made suffix runnable, but the measured S4/S8 results below closed it on correctness and performance.
...
- **BTC forum clarification (22/07)**: Round 2 intentionally scores **all 420 requests with no warm-up**. `failed_count` includes HTTP error, 120s end-to-end timeout, or connection/parse exception; no retry; each failed turn scores 0. Startup failure is not included. BTC explicitly permits custom images built from any vLLM image and permits packaged draft weights, provided they are not tuned to overfit the accuracy gate. Therefore the public requirements scrape's 330-scored/90-warmup wording is stale, and `replay_r2.py` must score all 420 for current calibration.
- **22/07 clean portal batch (all `config_hash=603c84…`, total 420, failed 5, warmup 0, tbt median 4ms)**: **022** `--performance-mode=interactivity` → **62.39**, TTFT 45/62ms; **023** `--max-num-batched-tokens=4096` → **61.32**, TTFT 49/62ms; **024** `--max-num-batched-tokens=2048` → **62.91**, TTFT 42/60ms; **025** `--attention-backend=FLASHINFER` → **60.52**, TTFT 51/71ms. Conclusions: interactivity is a small regression; 4096 is a clear regression; 2048 is statistically tied with 020 (−0.17, same p50/tbt, p95 +1); forced FlashInfer is a clear regression. The constant failed=5 across all four strengthens the case that these failures are not controlled by the tested kernel/scheduler flags.
- **config_hash decoded**: it is the HARNESS/environment version, NOT our config — 011–014 = `a84041…`, 016–025 = `603c84f…`, the switch aligns exactly with the 17→18/07 harness change (330→420, warmup 90→0). So config_hash never proves our flags took effect, and it confirms all 18/07+ runs share one harness.
...
- **submit_026** (22/07/2026 11:14): byte-identical rerun of 020 → **59.15**, TTFT p50 **53ms**, p95 **76ms**, tbt median 4ms, failed **4**, total 420. This is −3.93 points vs the identical 020 (63.08, 42/59ms) despite one fewer failure and the same `config_hash`. **Portal run-to-run/host-time variance is therefore much larger than the previously assumed ±1 band; TTFT alone drifted by 11ms p50 / 17ms p95 while TPOT stayed fixed.** Consequence: single-run deltas below ~2–4 points cannot reliably be attributed to a flag without interleaved baseline reruns, and failed_count is not the main score bottleneck.
- **Leaderboard published 24/07**: #1 Super Trio **83.32**; current rank-8 cutoff **69.44**; ranks 6–10 are packed within 0.49 points. Team best 020 = **63.08**, so reaching the current cutoff requires about +6.36 points; target **≥74** for a safer qualification margin. At TTFT≈42ms and ~5/420 failures, cutoff requires TPOT≈3.25ms and 73.67 points requires TPOT≈2.75ms; rerunning 020 alone cannot close the gap.
- **Suffix battery FINAL (24/07, 4090 MiG-sim) — REJECTED.** Stable bracket: D0A **70.20**, TPOT **2.49ms**, equivalence 6/6, needle 2/5 → S4 **65.45**, **3.02ms**, 0/6, 1/5 → S8 **65.49**, **3.04ms**, 0/6, 1/5 → D0B **70.08**, **2.49ms**, 6/6, 2/5. Baseline drift was only −0.12 points, so suffix causally loses ~4.7 points and slows TPOT 21–22%. S4 accepted 23.1% of drafted tokens / 0.497 accepted tokens per proposal; S8 accepted 20.7% / 0.504 per proposal — raising the max from 4→8 adds no useful accepted work. The 0/6 outputs include corrupted sequences/JSON and D0B exactly reproduces D0A, proving a hybrid-target correctness incompatibility rather than nondeterminism. **Do not run S16, publish the suffix image, or submit suffix.** Full postmortem: `tools/docs/SUFFIX_FINDINGS_20260724.md`.
- **Next active candidate: in-flight BitsAndBytes W4.** `tools/runpod/w4_battery_r2.sh` brackets `FP8A → BNB4A → BNB4B → FP8B`, saves startup/environment/needle data, checks BNB4 determinism across restart, and requires both BNB4 runs to beat the FP8 TPOT mean by ≥15% (local target ≤2.12ms) with higher ERS and zero errors. Cross-quantization text equality is informational only; W4 may legitimately change greedy outputs. If and only if the local gate passes, run BF16-vs-BNB4 GPQA via `ONLY="G1 G2" bash tools/evaluation/gpqa_r2.sh`. Runbook: `tools/docs/W4_RUNPOD.md`.
- **Final-5 REOPENED on 24/07** because the published cutoff is above every existing result. Retain 020, 024, and 018 BF16 insurance provisionally; reserve at least two slots for validated W4 or other audit-safe quantization candidates.
- **Current plan (24–30/07)**: ① run the bracketed BitsAndBytes W4 battery on the retained pod; ② reject it locally unless both W4 runs deliver ≥15% TPOT gain, higher ERS, zero errors, deterministic restart behavior, and needle no worse than FP8; ③ if it passes, measure BF16-vs-W4 GPQA Δ and require Δ≤0.10; ④ only then package/validate a public image and interleave its portal runs with exact 020 baselines; ⑤ ask BTC whether offline AWQ/GPTQ/Marlin representation of the exact mounted weights is permitted before opening that higher-engineering track.

Open question: are bodies still `temperature=0` (assumed). Custom images based on any vLLM image and packaged draft weights are now formally confirmed allowed by BTC (22/07 forum answer).

### Pod experiment protocol (round 2)

Same RunPod pattern as round 1 (`tools/runpod/pod_overnight.sh` header documents it): deploy `vllm/vllm-openai:v0.25.1` with the tiny-model keepalive as Container Start Command, filter hosts CUDA ≥ 13.0, clone the repo into `/workspace/repo`, download `LiquidAI/LFM2.5-1.2B-Instruct` to `/workspace/model` — **with `HF_HUB_DISABLE_XET=1`**: on a network-volume `/workspace` the default hf-xet path deadlocks on `.lock` files / hangs in "Reconstructing" (killed processes leave stale locks that block retries — `pkill -9 -f "hf download" && rm -rf /workspace/model` to recover; xet-disabled download takes ~16s). The current active entry point is **`bash tools/runpod/pod_install_bitsandbytes.sh && bash tools/runpod/w4_battery_r2.sh`**; it uses the same MiG simulation (3 cores via taskset + VRAM capped to 17.1GB). Historical batteries remain under `tools/runpod/`.

Replay is **`tools/replay/replay_r2.py`**: synthetic prompts sized in real tokenizer tokens (prompts are withheld by BTC), arrival `timestamp_ms` + closed-loop `think_ms=3000`, streaming TTFT/TPOT, and round-2 scoring constants. It now scores **all 420 requests by default**, matching the current BTC harness; `--legacy-exclude-marked-warmup` is only for historical 330-request comparisons. Two seeded prompt regimes bracket reality: `--mode shared` (per-conv common prefix ≈ 3.9k tokens → APC best case) and `--mode fresh` (new 4k tokens every turn → APC worst case). Compare turn-0 vs turn-1..5 TTFT p50 to see APC working. Local↔portal: ordering transfers, absolutes don't (round-1 lesson; 4090 ≈ 4–8× the slice).
...
- `tools/analysis/` — offline trace analysis and result summarizers; `tools/replay/` — replay clients; `tools/evaluation/` — correctness/needle/GPQA checks; `tools/runpod/` — pod setup, batteries, and image packaging; `tools/docs/` — experiment reports and runbooks. See `tools/README.md`.
- `docker/lfm25-suffix/` — archived reproducible suffix-image prototype. S4/S8 failed the 24/07 correctness and latency gates; do not build, publish, or use its compose template.
