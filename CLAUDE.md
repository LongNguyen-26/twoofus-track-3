...
- **INT4 weights — now the main track, first gate PASSED (25/07, H100 @ 12% MPS = 14 SMs)**: a batch-1 GEMV at an LFM2.5 MLP shape reaches **1011 GB/s, higher than a bulk 512MB reduction (871 GB/s)**, so 14 SMs are nowhere near saturated and decode is **bandwidth bound, not SM bound**, at MiG SM budgets. Halving weight bytes really does halve the weight-read term; only Marlin's dequant tax is still unknown. Caveat: MPS caps SMs but not the memory subsystem, so the rig runs ~1.5× the slice's bandwidth — judge by ratios, not absolute ms. It attacks only the 1.74ms bandwidth term. All linears at zero dequant tax ⇒ TPOT 3.32ms ⇒ **ERS ~68.8**; including the tied embedding/lm_head ⇒ 3.03ms ⇒ **~71.4**. That is *barely* the cutoff, in the perfect case, and Marlin's dequant spends SM cycles — the scarce resource. Keep as priority 2, gated on the microbenchmark.
