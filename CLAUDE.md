...
- **Remaining plan (22–30/07)**: ① do NOT burn submission slots on blind probes — only submit if a change is pod-validated (needle ≥ 2/5 + ERS win vs D0); ② **GPQA final-5 prep is now the main task** (~25–28/07): needs lm_eval on a pod + the gated `Idavidrein/gpqa` dataset — **user must create a fresh HF token (old one was revoked after the leak) and accept the GPQA gate on HF first**; measure Δ for the fp8 config (020/013) and the bf16 twin (018), then pick ≤5: 020, 013, 016, 019 (fp8 family) + 018 (bf16 tie-break hedge); ③ low-priority: investigate portal failed_count 4–7 (~1.5 pts) — unreproducible locally (all local runs errors 0/330), likely harness-side cold-start; ④ watch the leaderboard when published; tie-break ④ favors our early submissions.
