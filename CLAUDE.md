...
- **Plan for 22/07 (user returns, tops up balance)**: ① rent pod (manual deploy, CUDA ≥ 13.0 filter), run D0/D1/D2 draft-model battery + mandatory needle-in-haystack long-context check; ② if a draft wins locally, crane-append its weights into a public image (round-1 workflow, user pushes with their Docker Hub creds), verify e2e, then submit — keep `--enable-prefix-caching` on; ③ also investigate failed_count on the pod (per-request timeout? cold-start fails); ④ GPQA final-5 prep ~25–28/07.
