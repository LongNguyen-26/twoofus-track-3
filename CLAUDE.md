...
- **Untested lever found in the server's own config dump (25/07)**: v0.25.1 runs with `custom_ops: ['none']`, `ir_op_priority(rms_norm=['native'])` and `pass_config` `fuse_norm_quant: False` / `fuse_act_quant: False`. Those fuse RMSNorm+fp8-quant and SiLU+fp8-quant, targeting the ~1.9ms of TPOT that is neither weight bandwidth nor embedding — roughly 150 small kernels on 16 SMs. Batteries H5/H6/H7 in `h_battery_r2.sh` test them; estimated +1.5 to +2.5 points, stock image, no audit surface.
